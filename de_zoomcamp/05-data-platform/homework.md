...
- `.bruin.yml` and `pipeline/` with `pipeline.yml` and `assets/` [x]
...
- `time_interval` - incremental based on a time column [x]
...
- `bruin run --var 'taxi_types=["yellow"]'` [x]
...
- `bruin run ingestion/trips.py --downstream` [x]
